...
if git ls-files >& /dev/null &&  [[ -f ssh-copy-id.sh ]]; then
	...
	$SUDO curl -L https://raw.githubusercontent.com/arkitoure/ssh-copy-id-for-OSX/master/ssh-copy-id.sh -o /usr/local/bin/ssh-copy-id || { echo "Failed to install ssh-copy-id into /usr/local/bin."; exit 1; }
